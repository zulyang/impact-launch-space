...
public class JdbcResetTokenDAO implements ResetTokenDAO{
  ...
  public ResetToken retrieveResetToken(String username){
    ...
    try {
      ...
      if (rs.next()) {
        reset_token = new ResetToken(rs.getString("reset_code"), rs.getString("username"),rs.getTimestamp("expiry_date").toString());
      }
      rs.close();
      ps.close();
      return reset_token;
  ...
  public void updateResetCode(String resetCode, String username, String expiryDate) {
    ...
    try {
      DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
      Date date = (Date) dateFormat.parse(expiryDate);
